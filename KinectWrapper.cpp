...
KinectWrapper::KinectWrapper(void)
    : m_skeletons(0), m_hNextDepthFrameEvent(0), m_hNextVideoFrameEvent(0),
      ...
      m_mouse_x(0), m_mouse_y(0), m_pAudio(nullptr) {
...
void KinectWrapper::initialize() {
  // Initialize Kinect
  int status = NuiInitialize(NUI_INITIALIZE_FLAG_USES_AUDIO |
  ...
  // status = NuiImageStreamOpen( NUI_IMAGE_TYPE_DEPTH,
  // NUI_IMAGE_RESOLUTION_320x240, 0, 2, m_hNextDepthFrameEvent,
...
void KinectWrapper::DepthToWorld(int x, int y, int depthValue, float &rx,
                                 float &ry, float &rz) {
...
RGBQUAD KinectWrapper::KinNuiShortToQuadDepth(USHORT s) {
  ...
  USHORT Player = s & 7;
  // Transforme les informations de profondeur sur 13-Bit en une intensité codé
  // sur 8 Bits
