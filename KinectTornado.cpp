...
int main(int argc, char **argv) {
  ...
  ghDepth =
      clCreateBuffer(clContext, CL_MEM_READ_ONLY,
                     gDepthWidth * gDepthHeight * gKinectColorDepth, 0, 0);
#endif
  // --------------------------------------------------------------------------------
...
// kinectWrapper->getClick();
  ...
  if (depth)
    clEnqueueWriteBuffer(clCommandQueue, ghDepth, CL_FALSE, 0,
                         gKinectColorDepth * gDepthWidth * gDepthHeight, depth,
                         0, 0, 0);
...
// glRotatef(anim*9, 0.0, 0.0, 1.0);
  ...
  for (int y(0); y < kinectWrapper->gVideoHeight; y += step) {
    for (int x(0); x < kinectWrapper->gVideoWidth; x += step) {
      int depthIndex =
          ((y / 2 - 12) * kinectWrapper->gDepthWidth + (x / 2 - 12)) *
          kinectWrapper->gKinectDepthDepth;
      if (depthIndex >= 0) {
        depthIndex = depthIndex %
                     (kinectWrapper->gDepthWidth * kinectWrapper->gDepthHeight *
                      kinectWrapper->gKinectDepthDepth);
        ...
        if (drawPoint) {
          ...
          if (d != 0 || gScale == 0.f) {
            if (gActiveColor) {
              int index = (y * kinectWrapper->gVideoWidth + x) *
                          kinectWrapper->gKinectVideoDepth;
            ...
            glVertex3f((x - (kinectWrapper->gVideoWidth / 2)) * 0.005f,
